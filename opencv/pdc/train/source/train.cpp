...
int main(int argc, char const *argv[])
{
	...
	if(TRAIN)
	{
		...
		for(int num=0; num<PosSamNO && getline(finPos,ImgName); num++)
		{
			...
			ImgName = POS_SAMPLE_PATH+ImgName;		//加上正样本的路径名
			Mat src = imread(ImgName);	//读取图片
			if(CENTRAL_CROP)
				if(src.cols==96 && src.rows==160)
				src = src(Rect(16, 16, 64, 128));
			resize(src,src,Size(64,128)); //训练样本已经按照长：宽 = 2：1 直接缩放就好
		...
		{
			cout<<"处理负样本："<<ImgName<<endl;

 		        ImgName = NEG_SAMPLE_PATH+ImgName;		//加上负样本的路径名
			Mat src = imread(ImgName);	//读取图片
		...
		if(HardExampleNO > 0)
		{
			...
			{
				cout<<"处理难例："<<ImgName<<endl;
			        ImgName = HARD_SAMPLE_PATH + ImgName;//加上HardExample负样本的路径名
				Mat src = imread(ImgName);//读取图片
	...
	/*	ofstream fout("SampleFeatureMat.txt");
		...
		*/
		...
		cout<<"训练完成, 模型数据已经保存在SVM_HOG.xml"<<endl;
